...
class TestGoogleServices(unittest.TestCase):
    def test_google_maps_service_helper(self):
        ...
        self.assertEqual(gmsh.query, "https://maps.googleapis.com/maps/api/geocode/json?address=query&key=key")
        bb = BoundingBox()
        ...
        self.assertEqual(gmsh.query, "https://maps.googleapis.com/maps/api/geocode/json?address=two+words&key=key&bounds=1.0,0.0|0.0,1.0")

    def test_google_maps_response_parser_valid(self):
        gmsrp = GoogleMapsServiceResponseParser()
        fake_response = {"status": "OK", "results": [ {"formatted_address": "Addr", "geometry": {"location": {"lat": "1.0", "lng": "2.0"}}}]}
    ...
    def test_google_maps_response_parser_invalid(self):
        ...
        fake_response = {"status": "OK", "results": [ {"formatted_address": "Addr", "geometry": {"location": 1}}]}
        out = gmsrp.parse(fake_response)
...
class TestHereServices(unittest.TestCase):
    def test_here_service_helper(self):
        ...
        self.assertEqual(hsh.query, "https://geocoder.cit.api.here.com/6.2/geocode.json?app_id=appid&app_code=appcode&searchtext=query")
        bb = BoundingBox()
        ...
        self.assertEqual(hsh.query, "https://geocoder.cit.api.here.com/6.2/geocode.json?app_id=appid&app_code=appcode&searchtext=two+words&bounds=0.0,0.0;1.0,1.0")

    def test_here_response_parser_valid(self):
        hsrp = HereServiceResponseParser()
        fake_response = {"Response": {"View": [{"Result":[ {"Location": {"Address": {"Label": "Addr"}, "DisplayPosition": {"Latitude": 1.0, "Longitude": 2.0}}}]}]}}
    ...
    def test_here_response_parser_invalid(self):
        ...
        fake_response = {"Response": {"View": [{"Result":[ {"Location": {"Address": {"Label": "Addr"}, "Display_Position": {"Latitude": 1.0, "Longitude": 2.0}}}]}]}}
        out = hsrp.parse(fake_response)
        self.assertIsNone(out)
